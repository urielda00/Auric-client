      ...
      <Pressable style={styles.sheet} onPress={() => {}}>
        ...
        {current ? (
          <View style={styles.nowPlayingWrap}>
            <LinearGradient colors={['rgba(167,140,240,0.16)', 'rgba(90,209,224,0.08)']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.nowPlayingCard}>
              ...
              <EqualizerBars bars={3} height={16} width={2.5} gap={3} color={colors.violetLight} active={isPlaying} />
            </LinearGradient>
          </View>
        ) : null}

        <Eyebrow style={{ paddingHorizontal: 18, marginBottom: 12, letterSpacing: 1.7 }}>Next up · drag to reorder</Eyebrow>
        ...
        >
          {items.length ? (
            <DraggableQueueList items={items} onReorder={move} onPlay={handlePlay} onRemove={(item, index) => removeAt(index)} />
          ) : (
            <Text style={styles.emptyText}>{'Nothing queued. Add tracks with "Play Next" or start a shuffle.'}</Text>
          )}
        </ScrollView>
      </Pressable>
    </Pressable>
