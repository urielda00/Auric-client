...
export default function StatsScreen() {
  ...
  const loadStats = useCallback(async () => {
    ...
    setStatus("loading");
    try {
      const next = await statsService.getStats();
      if (generation !== requestGeneration.current) return;
      useLibraryStore
        .getState()
        .cacheTracks([
          ...next.topTracks.map((item) => item.track),
          ...next.rediscovered,
        ]);
      setStats(next);
      setStatus(next.cached ? "cached" : "success");
    } catch {
    ...
    <SafeAreaView style={styles.screen} edges={["top"]}>
      ...
      >
        {stats ? (
          <>
            <LinearGradient
              colors={gradients.statsHero}
              locations={gradients.statsHeroLocations}
              start={{ x: 0.05, y: 0 }}
              end={{ x: 0.9, y: 1 }}
              style={styles.hero}
            >
              ...
              <Text style={styles.heroCaption}>
                {stats.meaningfulPlays} meaningful plays ·{" "}
                {stats.completedPlays} completed
              </Text>
            </LinearGradient>
            ...
            <View style={styles.section}>
              <Eyebrow style={{ marginBottom: 14, letterSpacing: 1.7 }}>
                Most played
              </Eyebrow>
              <View style={{ gap: 13 }}>
            ...
            <View style={[styles.section, { paddingBottom: 30 }]}>
              <Eyebrow style={{ marginBottom: 12, letterSpacing: 1.7 }}>
                Rediscovered lately
              </Eyebrow>
              {stats.rediscovered.length ? (
                ...
                <Text style={styles.emptyText}>
                  {
                    "Play something you haven't heard in a while — it'll show up here."
                  }
                </Text>
              )}
            </View>
          ...
          <Pressable
            onPress={status === "error" ? loadStats : undefined}
            style={styles.statusWrap}
          >
            <Text style={styles.emptyText}>
              {status === "error"
                ? "Stats are unavailable. Tap to retry."
                : "Adding up your listening…"}
            </Text>
          </Pressable>
        )}
        {stats && status === "cached" ? (
          <Text style={styles.cacheNotice}>
            Showing the last saved Stats while offline.
          </Text>
        ) : null}
      </ScrollView>
