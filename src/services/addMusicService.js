...
const addMusicApi = serverApi ? createAddMusicApi(serverApi) : null;
...
function textArray(value, field, { required = false } = {}) {
  if (!Array.isArray(value) || (required && value.length === 0)) {
    throw new Error(
      `${field} must be ${required ? "a non-empty" : "an"} array`,
    );
  }
...
function normalizeInput(raw) {
  ...
  if (
    raw.release_year !== null &&
    (!Number.isInteger(raw.release_year) ||
      raw.release_year < 1 ||
      raw.release_year > 9999)
  ) {
