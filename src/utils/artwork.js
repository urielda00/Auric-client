 ...
 * Ported verbatim from `Mobile app design discussion/design_handoff_auric/artwork.js`.
 * Same title+artist (or an explicit visualSeed) always yields the same cover — no network,
 * no album images. Values are precomputed as hex so react-native-svg can consume them
 * directly (RN cannot parse oklch()).
 */
 ...
 * @param {string} title
 * @param {string} artist
 * @param {number|string} [seed] optional explicit visual seed overriding the title/artist hash
 * @returns {{hue:number, hue2:number, blobA:string, blobB:string, base:string, deep:string, pattern:string, letter:string}}
 */
export function trackArt(title, artist, seed) {
  const h = seed !== undefined && seed !== null ? hashTrack(seed, '') : hashTrack(title, artist);
  const hue = h % 360;
...
/*
Reference CSS the prototype uses (ported to react-native-svg in components/TrackArt.js):
